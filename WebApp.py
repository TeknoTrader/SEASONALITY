...
def main_page():
    ...
    # The user can select the colors of the page
    with st.sidebar:
        on_button = st.toggle("Choose colors")
        if (on_button):
            text_color = st.color_picker("TEXT COLOR", value="#FFFFFF")
            label_color = st.color_picker("LABEL COLOR", value="#FFFFFF")
            background_input = st.color_picker("BACKGROUND COLOR", value="#000000")

        else:
            # Set predifined colors
            label_color = "#FFFFFF"
            text_color = "#FFFFFF"
    ...
    # Applica il colore al label degli input numerici
    ...
    def main():
        ...
        def Represent(Mese, i, selections, db_selections):
            ...
            if db_selections[db_key] == "For CSV download":
                ...
                st.write(table1.style.pipe(style_table).to_html(classes=['dataframe', 'col0', 'col1', 'col2', 'col3'],
                                                                escape=False),
                         unsafe_allow_html=True)
        ...
        if rad == "Extended":
            ...
            if rep == "Image":
                Text3("⚠️OVERALL AVERAGE RETURN MONTHS:")
                data = dict(zip(np.array(Months_to_consider), np.array(MesiComplessivi)))
                ...
                # Barre colorate in base al valore
                ...
                # Aggiunta della legenda per le barre colorate
                ...
                # Converti i valori in decimali se non lo sono già
                ...
                # Definizione larghezza e altezza del grafico
                W = 400
                H = 400
                ...
                # Crea l'area di riempimento per la deviazione standard
                fill_area = base.mark_area(opacity=0.2, color='gray').encode(
                ...
                # SECOND CHART
                df = pd.DataFrame({
                    'Months': Months_to_consider,
                    'WinRate': [wr / 100 for wr in WRComplessivi]  # Dividi per 100 per ottenere il valore corretto
                })

                # Assicuriamoci che l'ordine dei mesi sia mantenuto
                ...
                base = alt.Chart(df).encode(
                    ...
                    # Modifica per mostrare % sull'asse
                )
                ...
                bars = base.mark_bar().encode(
                    ...
                    tooltip=[
                        ...
                        # Il formato .2% moltiplica il valore per 100 e aggiunge il simbolo %
                    ]
                )

                # Aggiungi le linee verticali
                line_40 = alt.Chart(pd.DataFrame({'x': [0.4]})).mark_rule(color='red').encode(
                    x='x')  # Convertito in decimale
...
selezione_pagina = st.sidebar.selectbox("Select the section", pagine)


# Pages creation
if selezione_pagina == "Home":
    main_page()
elif selezione_pagina == "Credits":
    credits()
